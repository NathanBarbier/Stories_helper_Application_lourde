...
{
    public partial class ProjectPaginationTeam : UserControl
    {
        ...
        public ProjectPaginationTeam(int idProject, int page = 0, string name = null)
        {
            InitializeComponent();
            ...
            int NbTeam = TeamRepository.GetTeamsByProjects(Session.UserId, true, true, page, name, false).Count;
        ...
        private void goToPaginateTeam(object sender, EventArgs e)
        {
            ...
            {
                MainProjectListTeam.goToPaginateTeam(idProject, pagination, name);
            }
            else if (Convert.ToString(button.Name) == "Forward")
            {
                if (page < pagination)
                {
                    MainProjectListTeam.goToPaginateTeam(idProject, page + 1, name);
                }
            }
            ...
            {
                MainProjectListTeam.goToPaginateTeam(idProject, 0, name);
            }
            else
            {
                MainProjectListTeam.goToPaginateTeam(idProject, Convert.ToInt32(button.Name), name);
            }
        }
    }
}
